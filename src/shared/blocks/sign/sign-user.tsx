  ...
  return (
    <>
      {isCheckSign || !mounted ? (
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            ...
            {user.isAdmin && (
              <>
                <DropdownMenuItem asChild>
                  <Link className="w-full cursor-pointer flex justify-center" href="/admin">
                    {/* <LayoutDashboard /> */}
                    {t('admin_title')}
                  </Link>
                </DropdownMenuItem>
        ...
        <div className="flex flex-col space-y-3 sm:flex-row sm:gap-3 sm:space-y-0 md:w-fit md:w-full">
          <button
            className={cn(
              'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all',
              'disabled:pointer-events-none disabled:opacity-50',
              'outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]',
              'hover:bg-accent hover:text-accent-foreground dark:hover:bg-[#24632e] dark:hover:text-white',
